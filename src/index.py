...
def search(text):
    ...
    if text != "":
      return {"sucesso": true, "resultado": { novel_id:resp[novel_id] for novel_id in resp if urllib.parse.unquote_plus(text.lower()) in str(resp[novel_id]).lower() }}
    else:
      return {"sucesso": true, "resultado": resp}
